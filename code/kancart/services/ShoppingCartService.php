...
class ShoppingCartService {
    ...
    private function displayAttribute($product_id, $dis_att) {
        ...
        if (!empty($dis_att)) {
            ...
            foreach ($dis_att as $key => $value) {
                $display_attribute .= ' - ' . $product_options[$key]['option_name'] . ':' . $product_options[$key]['variants'][$value]['variant_name'] . '<br>';
            }
        }
    ...
    public function getPriceInfos() {
        ...
        if ($this->cart['shipping_required'] == true && Registry::get('settings.General.estimate_shipping_cost') == 'Y') {
        ...
        if ($this->cart['use_gift_certificates']) {
        ...
        $priceInfos[] = array(
            'title' => fn_get_lang_var('total_cost'),
            'type' => 'total',
            'price' => convert_price($this->cart['total']),
            'currency' => CART_SECONDARY_CURRENCY,
            'position' => $position++
        );
    ...
    public function add($product_data) {
        $auth = &$_SESSION['auth'];
        $result = fn_add_product_to_cart($product_data, $this->cart, $auth);
    ...
    public function update($arr) {
        ...
        if (isset($this->cart['products'][$cart_item_id])) {
            ...
            if ($qty != 0) {
                ...
                fn_add_product_to_cart($product_data, $this->cart, $auth, true);
            }
            fn_save_cart_content($this->cart, $auth['user_id']);
        }
