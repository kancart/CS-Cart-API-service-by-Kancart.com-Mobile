...
class ReviewService {
    ...
    public function getAvgRatingScore($productId) {
        if ($productId) {
            return fn_get_average_rating($productId, 'P');
        }
    }

    public function getReviews($product, $pageNo = 0, $pageSize = 10) {
        $reviews = array();
        ...
        if (!Registry::is_exist('addons.discussion.status') || Registry::get('addons.discussion.status') == 'A') {
            ...
            if (!empty($discussion['thread_id'])) {
                ...
                $rows = fn_get_discussion_posts($discussion['thread_id'], $pageNo, $limit);
            }
        }
