...
class OrderService {
     ...
     * @return type 
     */
    public function getOrderInfos($userId, $pageNo, $pageSize) {
        list($orders, $count) = $this->getUserOrders($userId, $pageNo, $pageSize);
        ...
        foreach ($orders as $order) {
            ...
            $extractToOrder['uname'] = $orderDetail['email'];
    ...
    public function getUserOrders($userId, $pageNo, $pageSize) {
        ...
        return array($orders, $count['total']);
    }
    ...
    private function extractOrderStatus($order) {
        ...
        if ($row) {
            ...
            $status['date_added'] = date('Y-m-d H:i:s', $order['timestamp']);
            $status['position'] = 1;
    ...
    private function extractShippingMethod($orderDetail) {
        ...
        $orderShippingMethod['title'] = $value['shipping'];
    ...
    private function getOrderItems($orderDetail, $currency = CART_SECONDARY_CURRENCY) {
        ...
        if ($goodsList) {
            foreach ($goodsList as $goods) {
                ...
                $orderItem['final_price'] = convert_price($goods['price'], $currency);
...
//        }
    ...
    private function extractShippingAddress($orderDetail) {
        ...
        $address['gender'] = $orderDetail['s_title'];
    ...
    private function extractBillingAddress($orderDetail) {
        ...
        $address['gender'] = $orderDetail['b_title'];
